...
class JavaTimestampWithTimeZoneColumnType(zoneOffset: ZoneOffset) : ColumnType(), IDateColumnType {
    override val hasTimePart: Boolean = true
    private val calendarTimeZoneInstance = Calendar.getInstance(TimeZone.getTimeZone(zoneOffset))
    private val dateTimeStringFormatter =
        DateTimeFormatter.ISO_LOCAL_DATE_TIME.withLocale(Locale.ROOT).withZone(zoneOffset)
 ...
 */
fun Table.timestampWithTimeZone(name: String, zoneOffset: ZoneOffset = ZoneOffset.UTC): Column<Instant> =
    registerColumn(name, JavaTimestampWithTimeZoneColumnType(zoneOffset))